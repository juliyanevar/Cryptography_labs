...
namespace lab7
{
    public class DES_EEE2CryptoDecode : DESCryptoAbstract
    {
        public String Decode(String data, String key1, String key2, bool isParallel = false)
        {
            string result;
            result = Encoding.GetEncoding(1251).GetString(Decode(Encoding.GetEncoding(1251).GetBytes(data), key1, isParallel));
            result = Encoding.GetEncoding(1251).GetString(Decode(Encoding.GetEncoding(1251).GetBytes(result), key2, isParallel));
            result = Encoding.GetEncoding(1251).GetString(Decode(Encoding.GetEncoding(1251).GetBytes(result), key1, isParallel));
            return result;
        }
